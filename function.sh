...
function getComposer() {
    if $(which php > /dev/null 2>&1) && [[ -e composer.json ]]; then
        ...
        php bin/composer.phar update --prefer-source --optimize-autoloader --no-interaction --profile
        php bin/composer.phar install --prefer-source --optimize-autoloader --no-interaction --profile
    else
